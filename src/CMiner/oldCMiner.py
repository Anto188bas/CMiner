        ...
        for i in range(len(self.df)):
            ...
            while j >= 0:
                ...
                if self.is_subset(row, row_to_compare):
                    ...
                    # print_blue("zauu Location", location)
                    # print_blue("zauu Location to compare", location_row_to_compare)
...
class Pattern(MultiDiGraph):
    ...
    def apply_node_extension(self, extension: NodeExtension) -> 'Pattern':
        ...
        # if sum(len(new_pattern.pattern_mappings.mappings(g)) for g in new_pattern.graphs()) == 0:
        #     print_green(new_pattern)

        # Update the pattern mappings
        for target in extension.graphs():
            new_mappings = []
            for target_map in self.pattern_mappings.mappings(target):
                print_blue(target_map._retrieve_node_mapping())
                target_node_ids = extension.target_node_ids(target, target_map)
                # print_blue("target_node_ids", target_node_ids)
                ...
                if len(target_node_ids) == 0:
                    print_red("NOT EXTENDED")
                    continue

                print_blue(target_node_ids)

                # print_yellow("aaaa", target_map._retrieve_node_mapping())
                ...
                for target_node_id in target_node_ids:
                    ...
                    if not br_cond_node.check(new_pattern_new_node_id, target_node_id):
                        # print_red("new_pattern_new_node_id: ", new_pattern_new_node_id)
                        # print_red("target_node_id: ", target_node_id)
                        print_red("BREAKING CONDITION")
                        # print_red(self)
                        continue
...
class CMiner:
    ...
    def output(self, pattern):
        ...
        if len(pattern.nodes()) >= self._min_nodes:
            if self.output_path is not None:
                print("New solution")
                print(pattern.__str__(self.show_mappings, self.is_directed, self.with_frequencies), file=output_file)
            else:
                print(pattern.__str__(self.show_mappings, self.is_directed, self.with_frequencies))
