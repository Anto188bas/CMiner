...
class BitMatrix(ABC):
    ...
    def show(self):
        ...
        for i in range(len(self.matrix)):
            ...
            row_to_print.extend(self._split_bitmap(i)) # appending the bitmap splitted in the four parts
            data.append(row_to_print)
        table = tabulate(data, headers, tablefmt="fancy_grid")
        print(table)
        
    def _split_bitmap(self, row_num):
...
class QueryBitMatrix(BitMatrix):
    ...
    def compute(self):
        ...
        for edge in edges: 
            ...
            if(edge_to_compute not in self.matrix_indices):
                ...
                row_parts = self._split_bitmap(i) # taking the parts of the bitmap
    ...
    def _adapt_query_to_target(self, target_graph):
        ...
        for label in target_graph.get_all_edge_labels():
            self.graph.add_edge('dummy', 'dummy', label=label)
