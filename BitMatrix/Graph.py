...
class MultiDiGraph(nx.MultiDiGraph):
    ...
    def get_all_node_labels(self):
        return  sorted(set(flat_map([self.nodes[node]['labels'] for node in self.nodes])))

    def get_all_edge_labels(self):
        return  sorted(set([self.get_edge_data(edge[0], edge[1], edge[2])['label'] for edge in self.edges]))
    
    def get_edge_labels(self, source, destination):
        labels = []
        if(self.has_edge(source, destination)):
            labels.extend([edge_data.get('label') for edge_data in self[source][destination].values()])
    ...
    #         self.edge_labels = sorted(set([self.get_edge_data(edge[0], edge[1], edge[2])['label'] for edge in self.edges]))
